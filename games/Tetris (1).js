...
function startGame() {
  ...
  clearText();
  addText("LINES", { x: 1, y: 1, color: color`F` })
  addText("000", { x: 1, y: 2, color: color`2` })

  addText("SCORE", { x: 14, y: 1, color: color`F` })
  addText("000000", { x: 14, y: 2, color: color`2` })
...
function moveBlockDown() {
  if (currentGameState != gameStates.PLAYING) {
    addText("GAME", { x: 8, y: 5, color: color`0` })
    addText("OVER", { x: 8, y: 7, color: color`0` })
    addText("RESTART", { x: 7, y: 9, color: color`3` })
    addText("(j)", { x: 9, y: 10, color: color`3` })
    return;
  };
